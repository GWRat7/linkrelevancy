...
def analyse_mentions_and_links_semantic(
    ...
    Score each using SEMANTIC similarity:
      final_score = weight_local * sim(client_desc, local_context)
                  + weight_article * sim(client_desc, full_article)
    """
    ...
    # Safety: normalise weights in case they don't sum exactly to 1.0
    total_weight = weight_local + weight_article
    if total_weight > 0:
        weight_local = weight_local / total_weight
        weight_article = weight_article / total_weight
...
def main():
    ...
    # ---- Weight sliders (one input, one auto) ----
    ...
    def update_weights_from_local():
        wl = st.session_state["weight_local"]
        st.session_state["weight_article"] = round(1.0 - wl, 2)
    ...
    st.slider(
        ...
        step=0.05,
        key="weight_local",
        on_change=update_weights_from_local,
    )

    # This one moves automatically & is read-only
    st.slider(
        "Article-level context weight",
        min_value=0.0,
        max_value=1.0,
        step=0.05,
        key="weight_article",
    ...
    st.caption(
        f"Final score = {st.session_state['weight_local']:.2f} × local similarity "
        f"+ {st.session_state['weight_article']:.2f} × article similarity"
    )
    ...
    if submitted:
        ...
        weight_local = st.session_state["weight_local"]
        weight_article = st.session_state["weight_article"]
        ...
        try:
            ...
            # Helper functions that can see `url` from outer scope
            def get_linking_url(row):
                if row.get("url"):
                    return row["url"]
                # For pure mentions, use the page URL as the "linking" URL
                return url.strip()

            def get_text(row, max_len: int = 200):
                # Prefer anchor text for links
                anchor = row.get("anchor")
                if anchor:
                    return anchor
                # Otherwise fall back to a snippet of the context
                ctx = row.get("context", "")
                if len(ctx) <= max_len:
                    return ctx
                return ctx[:max_len] + "..."
